...
def gen_paras_report():
    ...
    proxy_a = 888
    paras = ["六、资产状况",
             f"1、截止{date}，四川省典当行业协会资产总额为{df_BS_1.loc['资产总计', '年末数']:,.2f}元，其中："
             ...
             f"2、四川省典当行业协会{period}费用{df_IS.loc['费用合计', '本年累计数']:,.2f}元，"
             ...
             "以下为附送8内容",
             "二、资产情况",
             f"截至{date}，四川省典当行业协会资产总额为{df_BS_1.loc['资产总计', '年末数'] / 10000:,.2f}万元，"
             ...
             f"截至{date}， 四川省典当行业协会负债总额为{df_BS_2.loc['负债合计', '年末数'] / 10000:,.2f}万元，较上年度增加{df_BS_2.loc['负债合计', '年末数'] / 10000 - df_BS_2.loc['负债合计', '年初数'] / 10000:,.2f}万元，"
             ...
             f"截至{date}，四川省典当行业协会净资产总额为{df_BS_2.loc['净资产合计', '年末数'] / 10000:,.2f}万元，"
